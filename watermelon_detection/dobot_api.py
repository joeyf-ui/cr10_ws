...
class DobotApi:
    ...
    def wait_reply(self):
        ...
        """
        data = self.socket_dobot.recv(2048)
...
class DobotApiDashboard(DobotApi):
    ...
    def EnableRobot(self,load,centerX,centerY,centerZ):
        ...
        Enable the robot
        """
        string = "EnableRobot({:f},{:f},{:f},{:f})".format(load,centerX,centerY,centerZ)
    ...
    def startDrag(self):
        command = "StartDrag()".format()
        self.send_data(command)
        return self.wait_reply()
    
    def stopDrag(self):
        command = "StopDrag()".format()
        self.send_data(command)
        return self.wait_reply()
    ...
    #ToDo: Account for table data type
    ...
    def SetCoils(self,index,addr,count,valTab):
        string = "SetCoils({:d},{:d},{:d},{:d})".format(index, addr, count, valTab)
        self.send_data(string)
        return self.wait_reply()
    ...
    def ModbusCreate(self, ip,port,slave_id,isRTU=0):
        """
        Creates a Modbus master and establishes connection to the Modbus Slave.
        Returns the master station index, supports 5 devices from 0-4
        Can be run using ModbusRTU over serial communication
        """
        command = "ModbusCreate({:s},{:d},{:d},{:d})".format(ip,port,slave_id,isRTU) if isRTU == 1 else "ModbusCreate({:s},{:d},{:d})".format(ip,port,slave_id)
        print(command)
        self.send_data(command)
        return self.wait_reply()
    ...
    def ModbusClose(self, index):
        """
        Disconnects with Modbus slave station
        """
        command = "ModbusClose({:d})".format(index)
        self.send_data(command)
        return self.wait_reply()
    
    def SetTerminal485(self,baudRate, dataLen, parityBit, stopBit):
        """
        
        """
        command = "SetTerminal485({:d},{:d},{:s},{:d})".format(baudRate, dataLen, parityBit,stopBit)
        self.send_data(command)
        return self.wait_reply()
...
class DobotApiMove(DobotApi):
    ...
    # [INFO][algs/src/common/command.cpp@OnDragTeachEnabled()#491] Command OnDragTeachEnabled executed(write)...demand:CT_MODE 
    # [INFO][algs/src/common/command.cpp@OnDragTeachEnabled()#491] Command OnDragTeachEnabled executed(write)...demand:CP_MODE 
    def OnDragTeachEnabled(self):
        command = "OnDragTeachEnabled()".format()
        self.send_data(command)
        return self.wait_reply()
...
########### Helper Functions #############
def parseResponse(feedback):
    # Format: ErrorID, Value, Command
    regex =  r"((?P<errorCode>\d+),\{(?P<values>.*)\},(?P<command>.*))"
    response = next(re.finditer(regex, feedback, re.MULTILINE))
    errorCode = response.group('errorCode')

##    if errorCode != '0':
##        print("Command ", response.group('command'), " failed with code ", errorCode)
##        raise Exception(errorCode)
    
    return response.group('values') #returns values as a string